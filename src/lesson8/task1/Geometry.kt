...
data class Point(val x: Double, val y: Double) {
    ...
    /**
     * Расчитать середину между двумя точками
     */
    fun mid(other: Point): Point = Point((x + other.x) / 2, (y + other.y) / 2)
...
fun diameter(vararg points: Point): Segment {
    ...
    var maxDist = 0.0
    var ans = Segment(points[0], points[1])
    for (i in 0 until points.size - 1) {
        for (j in i + 1 until points.size) {
            val distIToJ = points[i].distance(points[j])
            if (distIToJ > maxDist) {
                maxDist = distIToJ
...
fun circleByDiameter(diameter: Segment): Circle =
    Circle(diameter.begin.mid(diameter.end), diameter.begin.distance(diameter.end) / 2)
...
fun bisectorByPoints(a: Point, b: Point): Line {
    var angle = PI / 2
    if (a.x != b.x) angle = atan((b.y - a.y) / (b.x - a.x))
    return Line(a.mid(b), (angle + (PI / 2)) % PI) // отсекаем лишнее (% PI)
}
